    ...
    def data_passer(self): # decides which data to pass to client
		...
		passing["labels"]=la
                se = []
               	...
               	activity = self.get_data_for_activity(data,index,years,"Average Hours per Weekday",True)
               	for i in range (0, len(activity[1])):
                        temp_dict={}
                    	temp=float(activity[1][i])
                        temp_dict["label"]=years[i]
                        temp_dict["values"]=[temp]
                        se.append(temp_dict)
                passing["series"]=se
