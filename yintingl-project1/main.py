    ...
    def data_passer(self): # decides which data to pass to client
		...
		passing["labels"]=years
                se = []
               	...
               	activity_day= self.get_data_for_activity(data,index,years,"Average Hours per Weekday",True)
                activity_kend = self.get_data_for_activity(data,index,years,"Average Hours per Weekend and Holiday",True)
                temp_dict_day={}
                temp_dict_kend={}
                temp_list_day=[]
                temp_list_kend=[]
                temp_dict_day["label"]="Weekday"
                temp_dict_day["values"]=temp_list_day
                temp_dict_kend["label"]="Weekend"
                temp_dict_kend["values"]=temp_list_kend
                for i in range (0, len(activity_day[1])):
                        
                    	temp_day=float(activity_day[1][i])
                        temp_kend = float (activity_kend[1][i])
                        temp_list_day.append(temp_day)
                        temp_list_kend.append(temp_kend)
                se.append(temp_dict_day)
                se.append(temp_dict_kend)
                passing["series"]=se
