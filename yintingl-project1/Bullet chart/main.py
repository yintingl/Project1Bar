...
class MainHandler(BaseHandler):
    ...
    def data_passer(self): # decides which data to pass to client
		...
		years = ["2007"]
		passing = []
		index_range=[0,2,5,6,8]
		ranges=[[6,9,12],[1,2,4],[4,8,12],[2,6,10],[3.5,5.25,9]]
		markers=[[8],[2],[7],[3],[5]]
		for i in index_range:
                        activity = self.get_data_for_activity(data,i,years,
				"Average Hours per Weekday",True)
                        temp_dict={}
                        temp_dict["title"]=activity[0]
                        temp_dict["subtitle"]="Time, in hours"
                        temp_dict["ranges"]=ranges[index_range.index(i)]
                        temp=float(activity[1][0])
                        temp_dict["measures"]=[temp]
                        temp_dict["markers"]=markers[index_range.index(i)]
                        passing.append(temp_dict)
